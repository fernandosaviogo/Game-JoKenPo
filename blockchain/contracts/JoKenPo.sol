...
contract JoKenPo is IJoKenPo {
    ...
    constructor(){
        owner = payable(msg.sender);  // catch the contract owner.
    }

    // External it can only be called outside the contract (consumes less gas).
    ...
    function setBit(uint256 newBid) external {
    ...
    // Checks if the winner is repeated, saving it only once in the players array.
    function updateWinner(address winner) private {
        ...
        players.push(JKPLibrary.Player(winner, 1));  // Puts the winner in the player array
    }
    ...
    function play(JKPLibrary.Options newChoice) external payable  {
        ...
        require(msg.value >= bid, "Invalid bid"); // Validates the amount to be charged in the contract
    ...
    // It organizes the array of winners to display the ranking
